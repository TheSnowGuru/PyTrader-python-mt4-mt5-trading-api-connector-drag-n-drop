//+------------------------------------------------------------------+
//|                                      Pytrader_MT5_EA_V2.082.mq5 |
//|                           Copyright 2020, Deeptrade.ml / Branly. |
//|                                             https://www.mql5.com |
//+------------------------------------------------------------------+
#property copyright "Copyright 2021, Branly."
#property link      "https://www.mql5.com"
#property version   "2.082"
...
class MT5_F077;                           // reset sl & tp for position
class MT5_F078;                           // reset sl & tp for order
...
string MT5_F003::Execute(string command)
{
   ...
   if (SymbolInfoInteger(_symbol, SYMBOL_EXIST) == 0) {
   ...
   returnString = returnString + DoubleToString(_symbolInfo.SwapLong(), 5) + "^";
   returnString = returnString + DoubleToString(_symbolInfo.SwapShort(), 5) + "^";
   returnString = returnString + IntegerToString(_symbolInfo.StopsLevel()) + "^!";
...
string MT5_F004::Execute(string command)
{
   ...
   if (SymbolInfoInteger(_symbol,SYMBOL_EXIST) == 0)
   {
      returnString = "F998^2^NOK^00402^!";
   }
   else if (SymbolInfoInteger(_symbol, SYMBOL_VISIBLE) == 1)
   {
...
string MT5_F006::Execute(string command)
{
   string returnString = "";
...
class MT5_F008                                                      // check trading allowed
{
   private:
...
string MT5_F008::Execute(string command)
{
...
string MT5_F020::Execute(string command)
{
   ...
   if(SymbolInfoTick(_symbolInfo.Name(), last_tick)) {
      returnString = "F020^6^" + IntegerToString(last_tick.time) + "^" + DoubleToString(last_tick.ask, 5) + "^" + DoubleToString(last_tick.bid, 5) + "^" ;
      //double spread = MathAbs(_symbolInfo.Ask()- _symbolInfo.Bid());
...
string MT5_F021::Execute(string command)
{
   ...
   ResetLastError();
   iNbrOfRecords = CopyTicks(_symbolInfo.Name(), array, COPY_TICKS_ALL, 0, nbrOfTicks);
   
   if (iNbrOfRecords == -1) {
      returnString = "F998^2^NOK^02102^!";
      return returnString;
   }
   
   returnString = "F021^" + IntegerToString(iNbrOfRecords) + "^";
   for (int u = 0; u < iNbrOfRecords; u++) {
      returnString = returnString + IntegerToString(array[u].time_msc) + "$" + DoubleToString(array[u].ask,_digits) + "$" 
...
string MT5_F060::Execute(string command)
{
   ...
   for (int u = nbrOfOrders-1; u >= 0; u--) {
      ...
      returnString = returnString + IntegerToString(orderMagic) + "$" + DoubleToString(orderLots, 3) + "$" + DoubleToString(orderOpenPrice, 5) + "$";
...
class MT5_F061                                                       // get all positions
{
   private:
      // Other state variables
   public:
...
// -------------------------------------------------------------
// Constructor for a F061, get all positions
// -------------------------------------------------------------
MT5_F061::MT5_F061()
{
}
...
string MT5_F061::Execute(string command)
{
   ...
   for (int u = nbrOfPositions-1; u >= 0; u--) {
      ...
      //long     positionOrder           = PositionGetInteger()
      ...
      // find order ticket
      _dealTicket = 0;
      for ( int uu = 0; uu < ArraySize(deals); uu++) {
         if (deals[uu].positionTicket == positionTicket) {
            _dealTicket = deals[uu].ticket;
            //Print(_dealTicket);
            //break;
         }      
      }
      if (_dealTicket == 0) {_dealTicket = positionTicket;}
      
      returnString = returnString + IntegerToString(positionTicket) + "$" + positionSymbol + "$" + IntegerToString(_dealTicket) + "$";
...
// -------------------------------------------------------------
// Constructor for a F062, get closed positions within window
// -------------------------------------------------------------
MT5_F062::MT5_F062()
{
...
string MT5_F062::Execute(string command)
{
   ...
   for (int u = 0;  u < nbrOfHistoricalDeals; u++)
   {
      ...
      deals[iCounter].commission = dealCommission;
      deals[iCounter].time = dealTime;
      deals[iCounter].SL = sl;
      deals[iCounter].TP = tp;
   ...
   for (int u = 0; u < ArraySize(positions); u++){
      ...
      returnString = returnString + DoubleToString(positions[u].closePrice, 5) + "$"  + IntegerToString(positions[u].closeDate) + "$";
...
/////////////
class MT5_F063                                                       // get all closed positions
{
   private:
...
string MT5_F063::Execute(string command)
{
   string returnString = "";
   string split[];
   string split_2[];
   ...
   datetime firstDate = D'2015.01.01 00:00:00';      
   HistorySelect(firstDate, TimeCurrent());
   int nbrOfHistoricalDeals = HistoryDealsTotal();
   ...
   for (int u = 0;  u < nbrOfHistoricalDeals; u++)
   {
      ...
      //HistoryOrderSelect(orderTicket);
      long pos_ID=HistoryOrderGetInteger(orderTicket,ORDER_POSITION_ID);
      double sl = HistoryOrderGetDouble(pos_ID, ORDER_SL);
      //double sl = HistoryOrderGetDouble(orderTicket, ORDER_SL);
      double tp = HistoryOrderGetDouble(pos_ID, ORDER_TP);
      //double tp = HistoryOrderGetDouble(orderTicket, ORDER_TP);
...
class MT5_F064                                                       // get all deleted pendings within window
{
   private:
...
string MT5_F064::Execute(string command)
{
   ...
   HistorySelect(StructToTime(begin),StructToTime(end));
   selectDate = StructToTime(begin);
   int nbrOfOrders = HistoryOrdersTotal();   
   ...
   // count number of orders within settings
   int counter = 0;
   for (int u = nbrOfOrders-1; u >= 0; u--) {
   
      ulong    orderTicket          = HistoryOrderGetTicket(u);
      int      orderType            = HistoryOrderGetInteger(orderTicket, ORDER_TYPE);
      datetime orderOpenTime        = HistoryOrderGetInteger(orderTicket, ORDER_TIME_SETUP);
      datetime orderDeleteTime      = HistoryOrderGetInteger(orderTicket, ORDER_TIME_DONE); 
      if (orderType == ORDER_TYPE_BUY_STOP || orderType == ORDER_TYPE_SELL_STOP || orderType == ORDER_TYPE_BUY_LIMIT || orderType == ORDER_TYPE_SELL_LIMIT) {
         ...
         if (orderOpenTime >= beginDate && orderDeleteTime <= endDate)
         {
   ...
   returnString = "F064^" + IntegerToString(counter) + "^";
   ...
   for (int u = nbrOfOrders-1; u >= 0; u--) {
      ...
      double   orderLots            = HistoryOrderGetDouble(orderTicket, ORDER_VOLUME_INITIAL);
      double   orderSL              = HistoryOrderGetDouble(orderTicket, ORDER_SL);
      double   orderTP              = HistoryOrderGetDouble(orderTicket, ORDER_TP);
      double   orderOpenPrice       = HistoryOrderGetDouble(orderTicket, ORDER_PRICE_OPEN);
      double   orderDeletePrice     = HistoryOrderGetDouble(orderTicket, ORDER_PRICE_CURRENT);
      ...
      if (orderType == ORDER_TYPE_BUY_STOP || orderType == ORDER_TYPE_SELL_STOP || orderType == ORDER_TYPE_BUY_LIMIT || orderType == ORDER_TYPE_SELL_LIMIT) {
         ...
         if (orderOpenTime >= beginDate && orderDeleteTime <= endDate) {
            ...
            _comment = filterComment(orderComment);
            returnString = returnString + IntegerToString(orderMagic) + "$" + DoubleToString(orderLots, 3) + "$" + DoubleToString(orderOpenPrice, 5) + "$";
            returnString = returnString + IntegerToString(orderOpenTime) + "$" +DoubleToString(orderSL, 5) + "$" + DoubleToString(orderTP, 5) + "$";
            returnString = returnString + DoubleToString(orderDeletePrice, 5) + "$" + IntegerToString(orderDeleteTime) + "$" + _comment + "^";
         }
...
///////////////////
class MT5_F065                                                       // get all deleted pendings
{
   private:
...
string MT5_F065::Execute(string command)
{
   string returnString = "";
   string _comment = "";
   ...
   // count the wanted deleted pendings
   int counter = 0;
   for (int u = nbrOfOrders-1; u >= 0; u--) {
   ...
   returnString = "F065^" + IntegerToString(counter) + "^";
...
string MT5_F070::Execute(string command)
{
   ...
   //marketWatchOrder = (bool)StrToNumber(split[11]);
   ...
   if (orderVolume > _symbolInfo.LotsMax() || orderVolume < _symbolInfo.LotsMin())
   { 
      return "F998^3^NOK^07008^0^!";
   }
   
   if (orderType == ORDER_TYPE_BUY && orderStopLoss != 0.0)
   {
      if (orderStopLoss >= _symbolInfo.Ask()) return "F998^3^NOK^07006^0^!";
   }
   if (orderType == ORDER_TYPE_BUY && orderTakeProfit != 0.0)
   {
      if (orderTakeProfit <= _symbolInfo.Ask()) return "F998^3^NOK^07007^0^!";
   }
   if (orderType == ORDER_TYPE_SELL && orderStopLoss != 0.0)
   {
      if (orderStopLoss <= _symbolInfo.Bid()) return "F998^3^NOK^07006^0^!";
   }
   if (orderType == ORDER_TYPE_SELL && orderTakeProfit != 0.0)
   {
      if (orderTakeProfit >= _symbolInfo.Bid()) return "F998^3^NOK^07007^0^!";
   }
   ...
   if (orderType == ORDER_TYPE_BUY || orderType == ORDER_TYPE_SELL)
   {
      ...
      {
         //Print("In Order OK");
         returnString = "F070^3^OK^" + IntegerToString(openOrder.ticket) + "^" + IntegerToString(openOrder.resultCode) + "^!";
      }
      else
      {
         returnString = "F998^3^NOK^07008^"  + IntegerToString(openOrder.resultCode) + "^!";
         Print(returnString);
         Print(GetLastError());
         //Print("In Order NOK");
         Print(returnString);
   ...
   //Print("Just before open pending order" );
   if (orderType == ORDER_TYPE_BUY_LIMIT || orderType == ORDER_TYPE_SELL_LIMIT || orderType == ORDER_TYPE_BUY_STOP 
               || orderType == ORDER_TYPE_SELL_STOP)
   {
      //Print("Just before open pending order 2." );
      openOrder = openPendingOrder(_symbolInfo.Name(), orderType, orderVolume, orderSlippage, orderOpenPrice, orderStopLoss, orderTakeProfit, orderMagicNumber);
      
      if (openOrder.OK == true)
      { 
         returnString = "F070^3^OK^" + IntegerToString(openOrder.ticket) + "^" + IntegerToString(openOrder.resultCode) + "^!";
         //Print(returnString);
         return returnString;
      ...
      {
         returnString =  "F998^3^NOK^07010^" + IntegerToString(openOrder.resultCode) + "^!";
         //Print(returnString);
         return returnString;
      }
   }
...
// -------------------------------------------------------------
// Constructor for a F071, close position
// -------------------------------------------------------------
MT5_F071::MT5_F071()
{
}
...
string MT5_F071::Execute(string command)
{
   ...
   if(!_trade.PositionClose(ticket))
   {
      ...
      returnString = "F998^3^NOK^07102^"  + IntegerToString(_trade.ResultRetcode()) + "^!";
   }
   else
   {
...
// -------------------------------------------------------------
// Constructor for a F071, close position partly
// -------------------------------------------------------------
MT5_F072::MT5_F072()
{
}
...
string MT5_F072::Execute(string command)
{
   ...
   if (volume_to_close > _symbolInfo.LotsMax()) {return "F998^3^NOK^07203^0^!";}
   ...
   if(!_trade.PositionClosePartial(ticket, volume_to_close))
   {
      ::Print(__FUNCTION__,": > An error occurred when closing a position: ",::GetLastError());
      Print(_trade.ResultRetcode());
      returnString = "F998^3^NOK^07204^" + IntegerToString(GetLastError()) + "^!";
   }
   else
   {
...
string MT5_F073::Execute(string command)
{
   ...
   if(!_trade.OrderDelete(ticket))
   {
      ...
      Print(_trade.ResultRetcode());
      returnString = "F998^3^NOK^07302^" + IntegerToString(_trade.ResultRetcode()) + "^!";
   }
   else
   {
...
// -------------------------------------------------------------
// Constructor for a F075, update sl and or tp for position
// -------------------------------------------------------------
MT5_F075::MT5_F075()
{
}
...
string MT5_F075::Execute(string command)
{
   ...
   int nbrOfPositions = PositionsTotal();
   if (nbrOfPositions == 0) return "F998^3^NOK^07502^0^!";
   for (int u = 0; u < nbrOfPositions; u++)
   {
      ...
      if (positionTicket == ticket)
      {
         ...
         if(!_trade.PositionModify(ticket, sl, tp))
         {
            ...
            Print(_trade.ResultRetcode());
            return "F998^3^NOK^07503^" + IntegerToString(_trade.ResultRetcode()) + "^!";
         }
         else
         {
            return "F075^1^OK^!";
...
// -------------------------------------------------------------
// Constructor for a F073, update sl and or tp for order
// -------------------------------------------------------------
MT5_F076::MT5_F076()
{
}
...
string MT5_F076::Execute(string command)
{
   ...
   int nbrOfOrders = OrdersTotal();
   if (nbrOfOrders == 0) return "F998^3^NOK^07602^0^!";
   for (int u = 0; u < nbrOfOrders; u++)
   {
      ...
      if (orderTicket == ticket)
      {
         ...
         if(!_trade.OrderModify(ticket, _orderInfo.PriceOpen(), sl, tp, _orderInfo.TypeTime(),0,0))
         {
            ::Print(__FUNCTION__,": > An error occurred when modifying position: ",::GetLastError());
            Print(_trade.ResultRetcode());
            return "F998^3^NOK^07603^" + IntegerToString(_trade.ResultRetcode()) + "^!";
         }
         else
         {
            return "F076^1^OK^!";
...
string MT5_F077::Execute(string command)
{
   string returnString;
   string split[];
   ulong ticket;
   ...
   for (int u = 0; u < nbrOfPositions; u++)
   {
      ...
      if (positionTicket == ticket)
      {
         ...
         if(!_trade.PositionModify(ticket, 0.0, 0.0))
         {
            ::Print(__FUNCTION__,": > An error occurred when modifying position: ",::GetLastError());
            Print(_trade.ResultRetcode());
            return "F998^3^NOK^07703^" + IntegerToString(_trade.ResultRetcode()) + "^!";
         }
         else
         {
            return "F077^1^OK^!";
...
class MT5_F078                         // update sl & tp for order
{
   private:
      // Other state variables
...
string MT5_F078::Execute(string command)
{
   string returnString;
   string split[];
   ulong ticket;
   ...
   for (int u = 0; u < nbrOfOrders; u++)
   {
      ...
      if (orderTicket == ticket)
      {
         ...
         if(!_trade.OrderModify(ticket, _orderInfo.PriceOpen(), 0.0, 0.0, _orderInfo.TypeTime(),0,0))
         {
            ::Print(__FUNCTION__,": > An error occurred when modifying position: ",::GetLastError());
            Print(_trade.ResultRetcode());
            return "F998^3^NOK^07803^!" + IntegerToString(_trade.ResultRetcode()) + "^!";
         }
         else
         {
            return "F078^1^OK^!";
...
{    
   ...
   */
   _openResult.OK = true;
   _openResult.ticket = 10;
   _openResult.position_order_id = 11;
   _openResult.message = "";
   ...
   while(tryCounter < tryCounterMax)
   {   
      ResetLastError();   
      ZeroMemory(result);
      ...
      if(!OrderSend(request, result))   
      {
         ...
         _openResult.ticket = -1;
         tryCounter++;
         Print("Error in placing an order: " + result.retcode);
      }
      else
      {
         // wait for position
         ...
         {  
            ...
            if (waitCounter < 0) {
               ...
               _openResult.message = orderReturnCode(result.retcode);
               _openResult.resultCode = result.retcode;
               Print("Opening order timed out, errorcode: " + result.retcode);
...
_openOrder openPendingOrder( string instrument, ENUM_ORDER_TYPE type, double volume, int orderSlippage, double pendingPrice, double SL, double TP, long magicNumber)
{  
   int waitCounter = 10;
   bool OK;
   _openOrder openOrder;
   
   //Print("In open pending 1.");
   ...
   //Print("In open pending 3.");
   
   _trade.SetExpertMagicNumber(magicNumber);
   _trade.SetMarginMode();
   _trade.SetTypeFillingBySymbol(instrument);
   _trade.SetDeviationInPoints(orderSlippage);
        
   if (type == ORDER_TYPE_BUY_STOP)
   {
      OK = _trade.BuyStop(_volume, _price, instrument, sl, tp, ORDER_TIME_GTC, 0, NULL);
   }
   else if (type == ORDER_TYPE_SELL_STOP)
   {
      OK = _trade.SellStop(_volume, _price, instrument, sl, tp, ORDER_TIME_GTC, 0, NULL);
   }
   else if (type == ORDER_TYPE_BUY_LIMIT)
   {
      OK = _trade.BuyLimit(_volume, _price, instrument, sl, tp, ORDER_TIME_GTC, 0, NULL);
   }
   else if (type == ORDER_TYPE_SELL_LIMIT)
   {
      OK = _trade.SellLimit(_volume, _price, instrument, sl, tp, ORDER_TIME_GTC, 0, NULL);
   }
   ...
   while (waitCounter > 0)
   {
      ...
      else
      {
         ...
         if (waitCounter <= 0)
         {
            ...
            openOrder.OK = false;
            openOrder.ticket = -1;
            openOrder.resultCode = resultCode;
            openOrder.message = orderReturnCode(resultCode);
            return openOrder;
         }
         Sleep(20);
      }
...
ENUM_TIMEFRAMES getTimeFrame(int frame)
{
   ...
   if (frame == 49153) return PERIOD_MN1;
   return -1;
}
...
{
   if (code == 10006) return "Order reject.";
   if (code == 10004) return "Order requote.";
   if (code == 10008) return "Order placed.";
...
MT5_F006 mt5_f006 = MT5_F006();
MT5_F007 mt5_f007 = MT5_F007();
MT5_F008 mt5_f008 = MT5_F008();
...
MT5_F063 mt5_f063 = MT5_F063();
MT5_F064 mt5_f064 = MT5_F064();
MT5_F065 mt5_f065 = MT5_F065();
...
MT5_F077 mt5_f077 = MT5_F077();
MT5_F078 mt5_f078 = MT5_F078();
...
int OnInit()
{
   ...
   if(MQLInfoString(MQL_PROGRAM_NAME)!="Pytrader_MT5_EA_V2.081a")
   { 
...
string executeCommand(string command)
{
   ...
   {
      returnString = mt5_f006.Execute(command);
   }
   ...
   {
      returnString = mt5_f008.Execute(command);
   }
   ...
   else if (commandSplit[0] == "F062")
   {
      ...
      returnString = mt5_f063.Execute(command);
   }
   else if (commandSplit[0] == "F064") {
      returnString = mt5_f064.Execute(command);
   }
   else if (commandSplit[0] == "F065") {
      returnString = mt5_f065.Execute(command);
   }
   ...
   {
      returnString = mt5_f077.Execute(command);
   }
   else if (commandSplit[0] == "F078")
   {
      returnString = mt5_f078.Execute(command);
   }
   else
   {
