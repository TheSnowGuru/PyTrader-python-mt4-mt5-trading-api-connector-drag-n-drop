...
class Pytrader_API:
    ...
    def Get_all_deleted_orders(self) -> pd.DataFrame:
        ...
        orders = self.create_empty_DataFrame(self.columnsDeletedOrders, 'id')
        x = dataString.split('#')
        if str(x[0]) != 'F065':
            self.command_return_error = str(x[2])
            self.command_OK = False
        ...
        for value in range(0, len(x)):
            ...
            rowOrders = pd.Series({
                    'ticket': int( y[0]), 'instrument': self.get_universal_instrument_name(str(y[1])), 'order_type': str(y[2]), 'magic_number': int(y[3]), 
                    'volume': float( y[4]), 'open_price': float( y[5]), 'open_time': int(y[6]), 'stop_loss': float( y[7]), 'take_profit': float( y[8]), 
                    'delete_price': float( y[9]), 'delete_time': int(y[10]), 'comment': str(y[11])})
            orders = orders.append(rowOrders, ignore_index=True)

        self.command_OK = True
        return orders

    def Get_all_deleted_orders_within_window(self,
                                 date_from: datetime = datetime(2020, 3, 3, tzinfo = pytz.timezone("Etc/UTC")),
                                 date_to: datetime = datetime.now()) -> pd.DataFrame:
    ...
    def Get_all_open_positions(self) -> pd.DataFrame:
        ...
        Returns:
            ...
            instrument,
            position_type,
            order_ticket, for MT4 same value as ticket
            magic_number,
            volume/lotsize,
            open_price,
            open_time,
            ...
            rowClosedPositions = pd.Series(
                {
                    ...
                    'open_time': int( y[7]), 'stop_loss': float( y[8]), 'take_profit': float( y[9]), 'close_price': float(y[10]), 'close_time': int( y[11]), 
                    'comment': str( y[12]), 'profit': float( y[13]), 'swap': float( y[14]), 'commission': float( y[15])})
            ...
            rowClosedPositions = pd.Series(
                {
                    'ticket': int(y[0]), 'instrument': self.get_universal_instrument_name(str( y[1])), 'order_ticket': int( y[2]), 
                    'position_type': str( y[3]), 'magic_number': int( y[4]), 'volume': float( y[5]), 'open_price': float( y[6]), 
                    'open_time': int( y[7]), 'stop_loss': float( y[8]), 'take_profit': float( y[9]), 'close_price': float(y[10]), 'close_time': int( y[11]), 'comment': str( y[12]), 
