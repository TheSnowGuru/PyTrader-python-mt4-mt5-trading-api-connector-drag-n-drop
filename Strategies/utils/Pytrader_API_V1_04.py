...
class Pytrader_API:
    ...
    def Get_PnL(self,
        ...
        closed_positions = self.Get_all_closed_positions(date_from, date_to)
    ...
    def Get_last_x_bars_from_now(self,
        ...
        if (self.numberofbars > self.max_bars):
            ...
            for index in range(0, iloop):
                self.command = 'F042#4#' + self.get_broker_instrument_name(self.instrument_name_universal) + '#' + \
            ...
            rowPositions = pd.Series(
                {
                    ...
                    'magic_number': int(y[3]), 'volume': float( y[4]), 'open_price': float( y[5]), 'open_time': int(y[6]),
                     ...
                     'swap': float( y[11]), 'commission': float( y[12])})
    ...
    def Open_order(self,
        ...
        comment.replace('$', '')
        comment.replace('!', '')
        self.command = 'F070#8#' + self.get_broker_instrument_name(self.instrument_name_universal) + '#' + ordertype + '#' + str(volume) + '#' + \
            str(openprice) + '#' + str(slippage) + '#' + str(magicnumber) + '#' + str(stoploss) + '#' + str(takeprofit) + '#' + str(comment) + '#'
        #print(self.command)
        ok, dataString = self.send_command(self.command)
        if not ok:
            self.command_OK = False
