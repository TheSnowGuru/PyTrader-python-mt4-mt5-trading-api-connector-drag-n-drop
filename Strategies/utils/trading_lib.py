...
class Session():
    ...
    def __init__(self,
                 ...
                 end_minute: int = 30):
        """
        Class for defining a trade session period
            Args:
                active: session active
                start_hour: start hour for the session
                end_hour: end hour of the session
                start_minute: start minute for the session
                end_minute: end minute of the session
        """
        self.active = active
    ...
    def checkSession(self) -> bool:
        """
            Check if actual time within session period
        Args:
            None
        Returns:
            bool: True (actual time within session period) or False
        """
        ...
        if (start < end):
            if (timeInMinutes >= start and timeInMinutes < end):
                return True
            else:
                return False
        
        if (start > end):
            if (timeInMinutes >= end and timeInMinutes < start):
                return False
            else:
                return True
    ...
    def __init__(self,
        ...
        Class for checking Forex Factory(FF) news calendar.
            Args:
                ...
                update_in_minutes: update interval for retrieving news calendar from FF
