...
class Pytrader_API:             
    ...
    def Disconnect(self):
        ...
        try:
            if self.sock is not None:
                ...
                self.socket_error_message = 'Socket is already None.'
                return False
        except Exception as e:
            self.socket_error_message = f'Exception occurred: {str(e)}'
            return False
    ...
    def Get_bars_from_date_to_date(self, 
        ...
        self.command = 'F043^5^' + self.get_broker_instrument_name(self.instrument_name_universal) + '^'+ str(timeframe) + '^' + \
            self.date_from.strftime('%Y/%m/%d/%H/%M/%S') + '^' + self.date_to.strftime('%Y/%m/%d/%H/%M/%S') + '^'
        ...
        for value in range(0, len(x)):
            ...
            rowOrder = {
                    ...
                    'magic_number': int(y[3]), 'volume': float( y[4]), 'open_price': float( y[5]), 'open_time': int(y[6]),
                     'stop_loss': float( y[7]), 'take_profit': float( y[8]), 'delete_price': float(y[9]), 'delete_time': int( y[10]), 
                     'comment': str( y[11])
            }
