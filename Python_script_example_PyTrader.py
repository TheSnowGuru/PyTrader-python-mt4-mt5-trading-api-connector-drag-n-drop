...
##  Connected: boolean
Connected = MT.Connect(server='127.0.0.1', port=11111, instrument_lookup=brokerInstrumentsLookup)
...
##  (documentation reference 6) - (acc name, login, currency, demo,real, leverage, limit orders, margin calls, margin close , trade_allowed
##  datatype = dictionairy
...
##  documentation reference 7 - (balance, equity,profit, margin, margin level, free margin
##  DynamicInfo = dictionairy(float)
...
##  documentation reference 8 - (max_lotsize,min_lot, lot step, , point, ticksize, tickvalue
##  InstrumentInfo = dictionairy, if instrument not known at broker the value 'None' will be returned
...
##  documentation reference 9 - instruemnt, ask, bid, last, volume
##  LastTicket = dictionary
LastTicket = MT.Get_last_tick_info(instrument='GBPNZD')
print('')
...
##  documentation reference 10 - instruemnt, datetime, open, high, low, close, volume
##  ActualBar = dictionary
ActualBar = MT.Get_actual_bar_info(instrument='EURUSD', timeframe=MT.Get_timeframe_value('H4'))
...
##  LastTickets = data frame
LastTickets = MT.Get_last_x_ticks_from_now(instrument='EURUSD', nbrofticks=500)
...
NewOrder = MT.Open_order(instrument='EURUSD', ordertype='buy_stop', volume=0.01, openprice=1.136, slippage=10, magicnumber=2000, stoploss=1.12, takeprofit=1.14, comment='Test')
...
NewOrder = MT.Open_order(instrument='EURUSD', ordertype='buy', volume=0.01, openprice=0.0, slippage=10, magicnumber=2000, stoploss=0.0, takeprofit=0.0, comment='Test')
if (NewOrder == -1):  # opening order failed
    print(MT.order_error)
...
ChangePosition = MT.Set_sl_and_tp_for_position(ticket=53136604, stoploss=0.0, takeprofit=1.11001)
...
##  ChangeOrder = boolean
ChangeOrder = MT.Set_sl_and_tp_for_order(ticket=53136804, stoploss=0.0, takeprofit=1.12001)
...
AllClosedPositions = MT.Get_all_closed_positions(date_from=datetime(2020, 3, 3, tzinfo=timezone), date_to=datetime.now())
